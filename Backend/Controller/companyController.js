...
const signup = async (req, res) => {
  ...
  try {
    ...
    const { company_name, location, description, username, password } =
      req.body;
    const hashPassword = await bcrypt.hash(password, 11);
    const query =
      "INSERT INTO Company (company_name, location, description, username, password_hash) VALUES (?, ?, ?, ?, ?)";
